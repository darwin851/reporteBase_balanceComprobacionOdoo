...
class AccountTrialBalance(models.TransientModel):
    ...
    def view_report(self):
        account_domain = [('account_type', 'in', BALANCE_SHEET_TYPES)]
    ...
    def _get_report_data(self, accounts_data, total_amount, range):
        ...
        repor_data = {
            ...
            'lines': [{
                ...
                'visible': True,
                'columns': [
                    {'name': formatLang(self.env, item['initial_balance']), 'figure_type': 'monetary',
                     'no_format': float_round(item['initial_balance'], precision_digits)} if item[
                                                                                                 'initial_balance'] > 0 else column,
                    {'name': formatLang(self.env, item['initial_balance'] * -1), 'figure_type': 'monetary',
                     'no_format': float_round(item['initial_balance'], precision_digits) * -1} if item[
                                                                                                      'initial_balance'] < 0 else column,
                    {'name': formatLang(self.env, item['debit']), 'figure_type': 'monetary',
                     'no_format': float_round(item['debit'], precision_digits)},
                    {'name': formatLang(self.env, item['credit']), 'figure_type': 'monetary',
                     'no_format': float_round(item['credit'], precision_digits)},
                    {'name': formatLang(self.env, item['ending_balance']), 'figure_type': 'monetary',
                     'no_format': float_round(item['ending_balance'], precision_digits)} if item[
                                                                                                'ending_balance'] > 0 else column,
                    {'name': formatLang(self.env, item['ending_balance'] * -1), 'figure_type': 'monetary',
                     'no_format': float_round(item['ending_balance'], precision_digits) * -1} if item[
                                                                                                     'ending_balance'] < 0 else column,
                ]
            } for item in trial_balance
            ],
            'footer': {
                'name': 'Total',
                'columns': [
                    {'name': formatLang(self.env, total_initial_deudor), 'figure_type': 'monetary',
                     'no_format': float_round(total_initial_deudor, precision_digits)},
                    {'name': formatLang(self.env, total_initial_acreedor), 'figure_type': 'monetary',
                     'no_format': float_round(total_initial_acreedor, precision_digits)},
                    {'name': formatLang(self.env, total_cargos), 'figure_type': 'monetary',
                     'no_format': float_round(total_cargos, precision_digits)},
                    {'name': formatLang(self.env, total_abonos), 'figure_type': 'monetary',
                     'no_format': float_round(total_abonos, precision_digits)},
                    {'name': formatLang(self.env, total_ending_deudor), 'figure_type': 'monetary',
                     'no_format': float_round(total_ending_deudor, precision_digits)},
                    {'name': formatLang(self.env, total_ending_acreedor), 'figure_type': 'monetary',
                     'no_format': float_round(total_ending_acreedor, precision_digits)},
                ]
            }
        }
        return repor_data
    ...
    @api.model
    def _get_data(self, account_ids, journal_ids, date_to, date_from, only_posted_moves, hide_account_at_0,
                  fy_start_date):
        ...
        tb_initial_acc_bs = self.env['account.move.line'].read_group(
            domain=initial_domain_bs, fields=['account_id', 'balance', 'amount_currency:sum'],
            groupby=['account_id'])
        ...
        tb_period_acc = self.env['account.move.line'].read_group(
            domain=period_domain, fields=['account_id', 'debit', 'credit', 'balance', 'amount_currency:sum'],
            groupby=['account_id'])
        total_amount = {}
